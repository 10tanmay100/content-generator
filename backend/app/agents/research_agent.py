...
def build_research_agent() -> Agent:
    return Agent(
        ...
        tools=[WebSearchTool()],
        # Uses the tool-calling model (llama3.2:latest), not the reasoning
        # model, because this agent invokes web_search and Ollama's tool
        # support is more consistent on Llama-family models than on
        # reasoning-distilled models like deepseek-r1.
        llm=get_tool_llm(),
        verbose=True,
